<template>
    <div class="container">
        <div class="card-wrapper">
            <div class="card" :class="{ disable: !profile.enable }" v-for="profile in profileStore.profiles"
                :key="profile.id">
                <div class="img">
                ...
                <router-link :to="`/profile/${profile._id}`" class="detail-btn"><i class="fas fa-eye"></i> View
                    Profile</router-link>
                <router-link v-if="authStore.isAdmin" :to="`/admin/profile/${profile._id}`" class="detail-btn"
...
await callOnce(async () => {
    await profileStore.get({ isAdmin: route.name.includes('admin') });
});

const handleLoadMore = async () => {
    profileStore.setCurrentPage(profileStore.currentPage + 1);
    await profileStore.get({ isAdmin: route.name.includes('admin') });
}

const isShowLoadMore = computed(() => {
    return profileStore.currentPage < profileStore.pages;
})
